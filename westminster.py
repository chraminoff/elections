...
####################################################################################
# Based on https://github.com/slashme/parliamentdiagram/blob/master/westminster.py #
####################################################################################

import cgi, re, math, random, datetime, sys, os
import subprocess as SP

def wmdiagram(inputlist,**kwargs):
    svgfilename = 'wmparlpic.svg' # 'svgfiles/' + datetime.datetime.utcnow().strftime("%Y-%m-%d-%H-%M-%S-%f") + "-" + str(hash(inputlist) % ((sys.maxsize + 1) * 2))+'.svg'
    # if inputlist:
    #initialize dictionary of options - this will hold things like spot radius, spacing, width of blocks, etc.
    optionlist={'radius':0.4,'spacing':0.1,'wingrows':0,'centercols':0,'fullwidth':0,'cozy':1}
    ...
    for i in partylist:
        ...
        for g, n in sumdelegates.items(): #g is the group name; n is the seat count for that group.
            if re.search(g,i[2]):
                sumdelegates[g] += i[1]
    ...
    if not error:
        ...
        #print svgfilename
        SP.Popen([svgfilename], stdin=SP.PIPE, shell=True)

if __name__ == '__main__':
    wmdiagram('GOV,300,right,green;OPP,299,left,red;GOV,1,head,green')