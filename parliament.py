...
def render_svg(partyspecs):
	...
	with StringIO() as outfile:
		#Write svg header:
		outfile.write(u'<?xml version="1.0" encoding="UTF-8" standalone="no"?>\n')
		outfile.write(u'<svg xmlns:svg="http://www.w3.org/2000/svg"\n')
		outfile.write(u'xmlns="http://www.w3.org/2000/svg" version="1.0"\n')
		#Make 350 px wide, 175 px high diagram with a 5 px blank border
		outfile.write(u'width="360" height="185">\n')
		outfile.write(u'<g>\n')
		#Print the number of seats in the middle at the bottom.
		outfile.write(u'<text x="175" y="175" style="font-size:36px;font-weight:bold;text-align:center;text-anchor:middle;font-family:Sans">'+str(sumdelegates)+'</text>\n')
...
def display(svg):
    '''Convenience function to display a string as an SVG image, by saving it and displaying it in the default SVG viewer (usually the default browser).
    This makes the use of imagemagick unnecessary.'''
    with open('parlpic.svg','w') as wf:
        wf.write(svg)
    import subprocess as SP
    SP.Popen(['parlpic.svg'], stdin=SP.PIPE, shell=True)
